...
export function CategoryList({ categories, currency, onCategoryClick }: CategoryListProps) {
  const maxMinor = categories.length > 0 ? categories[0].totalMinor : 1;
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      {categories.map((cat) => {
        ...
        const barWidth = maxMinor > 0 ? (cat.totalMinor / maxMinor) * 100 : 0;

        return (
          <button
            key={cat.category}
            onClick={() => onCategoryClick(cat.category)}
            className="flex flex-col gap-1.5 px-2 py-2.5 transition-colors"
          ...
          >
            ...
            {/* Progress bar */}
            <div className="w-full h-1 overflow-hidden" style={{ background: "var(--bg-raised)", borderRadius: "var(--radius-2xl)" }}>
              <div
                className="h-full transition-all"
                style={{ width: `${barWidth}%`, background: config.color, borderRadius: "var(--radius-2xl)" }}
              />
            </div>
          </button>
        );
